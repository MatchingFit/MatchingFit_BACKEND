...
public class ScoreService {
    ...
    public List<CompetencyScoreDTO> sumScore(ResumeEmbeddingRequestDTO resumeEmbeddingRequestDTO){
        ...
        for (Competency competency : competencies){
            ...
            int count = 0; //키워드 갯수세기

            //해당 키워드를 확인하고, 키워드가 있다면 점수를 주는 방식
            for (Keyword keyword : keywords){
                //⭐️숫자로 변환 / ✅EmbeddingService 사용
                List<Double> keywordEmbedding = embeddingService.parseEmbedding(keyword.getEmbedding());
                //⭐️ 코사인 유사도 계산 / ✅EmbeddingService 사용
                double score = embeddingService.cosineSimilarity(resumeEmbedding, keywordEmbedding);
                keyword.updateScore(score); //추가, 키워드의 점수를 업데이트
                totalScore += score;
                count ++;
            ...
            competency.updateTotalScore(avgScore); //추가, 역량점수 업데이트

            //역량별로 점수 결과(해당 역량 키워드별 합산)
            ...
            competencyRepository.save(competency); //추가, db에 저장, 이미 있으면 변경
        }
